...
class GetPopularMoviesUseCase(private val repo: MovieRepo, private val scheduler: Scheduler) {

    fun execute(): Single<List<MovieItemUI>> =
        repo.getRemotePopularMovies().subscribeOn(scheduler).map { moviesRemote ->
